...
LineSeries::LineSeries(QObject* parent)
    : ChartSeries(SeriesType::Line, parent)
{
}
...
void LineSeries::setLineStyle(int style)
{
...
void LineSeries::setLineType(int type)
{
...
void LineSeries::setLineWidth(qreal width)
{
...
void LineSeries::setLineVisible(bool visible)
{
...
void LineSeries::setPointMarkersVisible(bool visible)
{
...
void LineSeries::setPointMarkersRadius(qreal radius)
{
...
void LineSeries::setCrosshairMarkerVisible(bool visible)
{
...
void LineSeries::setCrosshairMarkerRadius(qreal radius)
{
    if (!std::isfinite(radius)) {
        return;
    }
...
void LineSeries::setCrosshairMarkerBorderColor(const QColor& color)
{
...
void LineSeries::setCrosshairMarkerBorderWidth(qreal width)
{
    if (!std::isfinite(width)) {
        return;
    }
...
void LineSeries::setCrosshairMarkerBackgroundColor(const QColor& color)
{
...
void LineSeries::setData(const QVariantList& rows)
{
    ...
    for (const QVariant& rowValue : rows) {
        ...
        qWarning() << "QtFinCharts: skipped invalid line row" << rowValue;
        continue;
    }

    model().setLinePoints(std::move(parsed), SeriesType::Line, std::move(whitespaceTimes));
...
void LineSeries::update(const QVariantMap& row, bool historicalUpdate)
{
    LineDataPoint point;
    if (parseRow(row, &point)) {
        ...
        const bool inserted = model().updateLinePoint(point);
    ...
    TimePoint whitespaceTime = 0;
    if (!isWhitespaceRow(row) || !parseTime(row.value(QStringLiteral("time")), &whitespaceTime)) {
        qWarning() << "QtFinCharts: skipped invalid line update" << row;
        return;
    }
    if (!shouldAcceptUpdate(whitespaceTime, historicalUpdate)) {
        return;
    }
    const bool inserted = model().updateWhitespace(whitespaceTime, SeriesType::Line);
...
bool LineSeries::parseRow(const QVariantMap& row, LineDataPoint* point)
{
    if (!point) {
        return false;
    }
